...
def select_optimizer(optim, param, lr, wd):
    ...
    return optimizer
#raise Exception表示抛出异常，try:... exception conditions:...也是表示抛出异常的语句，避免程序运行遇到异常但是一直卡住出不来
...
#在执行parser.parse_arg()之前，在命令行中输入的参数都不会生效
#parser.add_argument()：运行程序时，给定参数，通过调用给定的参数来运行程序
def parse_parms():
...
#en.environ(str):字符串表示的环境的映像对象
def reserve_mem():
    try:
        gpuid = int(os.environ["CUDA_VISIBLE_DEVICES"])
    except:
        gpuid = -1
#os.popen表示获取控制台的输出内容，返回的是一个file对象，从一个文件打开一个管道
#当"nvidia-smi --query-gpu=memory.total,memory.used --format=csv,nounits,noheader"时，进行read、strip、replace、split等操作
#split表示给定分隔符对字符串进行切片
#安装cuda之后，用nvidia-smi指令来查看GPU信息
    smi = (
        os.popen(
...
#benchmark基准
    ...
    # Training function
    ###############################################################################
    #time.time()用于记录程序运行时间
    #enumerate是python内置函数，参数为可遍历或者可迭代的对象
    #autograd.detect_anomaly是python内置的自动微分库函数
    def train():
...
#scalar标量，tensor张量
#parse_unsqueeze表示去掉最外维的一层维度，parse_squeeze表示在最外维基础上加上一个维度
                    # eval log
