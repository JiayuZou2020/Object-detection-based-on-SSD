...
#easydict以属性的方式去访问字典的值
#单下划线表示这是表面上的私有变量，实际上外部可以访问，但是不希望外界去访问这些变量
#双下划线表示这是实质上的私有变量，外部不能访问，只有在函数内部才可以访问
#前后双下划线表示python内置特殊变量，哪里都可以访问，通常表示python自带的一些函数和方法，比如__init__函数通常用来定义类

#训练参数的设置，parameter configuration
__C = edict()
cfg = __C
...
# padding
#进行卷积操作的时候在原矩阵的周围补一圈0元素，保证卷积之后与原来的矩阵大小相同
__C.MODEL.padding = 1
...
# scale_list
# 卷积神经网络部分采用了不同大小的卷积核，大小分别从3x3变化到21x21
...
#检测优化算法：adam或者sgd，自适应动量预测方法addaptive moment estimation
#随机最速梯度下降法stochastic gradient decendent
...
# __C.TRAIN.TOPK = 512
__C.TRAIN.TOPK = 128
#为了提高训练速度，要把上面的TRAIN.TOPK改成128
"""
Image data options
"""
# View train sequence Mean and Std
#csv是图像文件，view函数相当于resize函数
__C.view = edict()
