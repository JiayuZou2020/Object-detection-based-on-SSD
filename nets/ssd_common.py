...
# jaccard_with_anchors()用于计算IOU
    ...
    # 上面得到的实际上是参考框相对于真实框的偏移量，为了能够还原出原来的框，要进行转换
...
def tf_ssd_bboxes_decode_layer(feat_localizations,
    ...
    ymax = cy + h / 2.
    xmax = cx + w / 2.
    # bboxes是边框的四个角点的值，bboxes的形式是[ymin,xmin,ymax,xmax]
...
def tf_ssd_bboxes_select(predictions_net, localizations_net,
    ...
    with tf.name_scope(scope, 'ssd_bboxes_select',
        ...
        # Concat results.
        # tf.concat(params,axis = 1)表示在第1维上拼接张量
        d_scores = {}
        d_bboxes = {}
