...
# np.argsort(a)函数输出a中元素从小到大排列元素的下标，np.argsort(-score)输出-score按照从小到大排列元素的下标，也就是score按照从大到小排列元素的下标
# bboxes_sort()输出得分前k大（400个）的预测框的类别、得分与四个角点坐标
# 将重叠部分面积与参考框面积的比值作为得分
    idxes = np.argsort(-scores)
